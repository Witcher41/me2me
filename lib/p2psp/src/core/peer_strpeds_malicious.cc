...
boost::asio::ip::udp::endpoint PeerStrpeDsMalicious::ChooseMainTarget(){
	...
	while (!re_found){
		if (std::find(attacked_peers.begin(), attacked_peers.end(), peer_list_[r]) == attacked_peers.end() and std::find(malicious_peers.begin(), malicious_peers.end(), peer_list_[r]) == malicious_peers.end()) {
		  ...
		  r = (r+1) % (int)peer_list_.size();
		  attempt++;
		}

		if (attempt >= (int)peer_list_.size() and std::find(malicious_peers.begin(), malicious_peers.end(), peer_list_[r]) == malicious_peers.end()){
			re = peer_list_[r];
                        re_found = true;
			attempt = 0;
		}
	...
	if (std::find(attacked_peers.begin(), attacked_peers.end(), re) == attacked_peers.end()){
  ...
  void PeerStrpeDsMalicious::AllAttack(){
	...
	RefreshRegularPeers();
	if (std::find(regular_peers_.begin(), regular_peers_.end(), main_target_) == regular_peers_.end()){
  ...
  void PeerStrpeDsMalicious::RefreshRegularPeers(){
	...
	if(regular_file.is_open()){
		...
		while (std::getline(regular_file, str))
		{
			...
			if (regular_peers_attacked_.size()*2 < peer_list_.size()){
			  if (std::find(peer_list_.begin(), peer_list_.end(), boost::asio::ip::udp::endpoint(address,port)) == peer_list_.end()){
			    regular_peers_attacked_.push_back(boost::asio::ip::udp::endpoint(address,port));
			  }
			}
		}
		regular_file.close();
...
void PeerStrpeDsMalicious::SendChunk(const ip::udp::endpoint &peer) {
  ...
  if (persistent_attack_) {
	  ...
	  } else if (std::find(regular_peers_attacked_.begin(), regular_peers_attacked_.end(), peer) != regular_peers_attacked_.end()) {
		  team_socket_.send_to(buffer(chunk), peer);
		  sendto_counter_++;
